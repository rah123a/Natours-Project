...
exports.protect=catchAsync(async (req,res,next)=>{
  ...
  console.log('authcontroller.protect');
    ...
    let token;
          ...
          if(!token){
              return next(new AppError('You are not  ,Please log in to get access',401));
          }

    //2.)Verficatiion token
  ...
  // 4) Cheack if user changed password after token was issued
 
  if (curretUser.changedPasswordAfter(decoded.iat)) {
    next(new AppError('Resently Password is changed! Please login again', 401));
  }
  req.user = curretUser;
  next();
});
...
const user=await User.findOne({email:req.body.email});
if(!user){
  return next(new AppError('Thers no user with email addresss',404))
}
...
try{
 ...
 res.status(200).json({
  ...
  user.PasswordResetExpires=undefined;
  await user.save({validateBeforeSave:false});
  console.log("error in forgotpassword is ....",err);
...
//console.log("date....is",Date.now());
  ...
  const user=await User.findOne({
    PasswordResetToken:hashedToken,
    PasswordResetExpires:{$gt:Date.now()-1000000}
  });
